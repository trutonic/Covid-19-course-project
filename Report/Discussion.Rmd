...
Overall, the report provides a clear reason for performing the tests and does not appear to require any changes to the model or analysis. However, there are a few major comments that include certain areas where the report can be improved. These comments include clarification of the use of certain variables and more detailed explanations of procedures. The conclusion provides possible reasons for a lack of association between the percentage change in GDP and the two variables, but only includes lack of up-to-date information, it should instead focus on real-world explanations for this outcome. The two-factor ANOVA model should include an interaction term whether or not it is significant because it is still worth mentioning. The descriptive analysis should include more selective models that are reflective of the two-factor model and exclude any that do not appear relevant. Another issue was the use of the word “impact,” which should not be used because we are not looking at causal inferences, only at associations so this should be changed. The introduction should also make note of the distinctions in Q2 and Q3 and the reasons these specific time periods were considered. A few minor issues include Section 4.1.1, where there are two very similar plots, one is just more zoomed in, so removing the plot that is less intuitive can be beneficial. There are also a few typos in certain sections such as 4.1.3 and 4.1.5 with specific misspelled words mentioned below. There is also a section above the background with code, but with explanation so one should be provided. There are also five questions that were raised in the discussion report listed below. 
