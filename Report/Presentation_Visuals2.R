...
covid_gdp2 = covid_gdp %>% select(Location, PopTotal, Change_3_months,prp_case_increase,case_increase)
covid_gdp2$rank_increase = rank(covid_gdp2$prp_case_increase)


prpcut_20 =  covid_gdp2 %>% filter(rank_increase == 20)
prpcut_20 = toString(prpcut_20$prp_case_increase)

prpcut_40 =  covid_gdp2 %>% filter(rank_increase == 40)
prpcut_40 = toString(prpcut_40$prp_case_increase)

prpcut_59 =  covid_gdp2 %>% filter(rank_increase == 59)
prpcut_59 = toString(prpcut_59$prp_case_increase)

covid_gdp2$increase_cat = cut(covid_gdp2$rank_increase, breaks = c(0,20,40,59), labels = c(1, 2, 3))
covid_gdp2 = covid_gdp2 %>% select(Location, PopTotal,case_increase,prp_case_increase, increase_cat, Change_3_months,)
...
cut_0 =  covid_gdp2 %>% filter(rank_gdp == 1)
cut_0 = toString(cut_0$GDP.per.capita)

cut_20 =  covid_gdp2 %>% filter(rank_gdp == 20)
cut_20 = toString(cut_20$GDP.per.capita)

cut_40 =  covid_gdp2 %>% filter(rank_gdp == 40)
cut_40 = toString(cut_40$GDP.per.capita)

cut_59 =  covid_gdp2 %>% filter(rank_gdp == 59)
cut_59 = toString(cut_59$GDP.per.capita)


covid_gdp2$gdp_cat = cut(covid_gdp2$rank_gdp, breaks = c(0,20,40,59), labels = c(1, 2, 3))
#covid_gdp2_copy = covid_gdp2
#covid_gdp2 = covid_gdp2 %>% select(Location, Change_3_months, increase_cat, gdp_cat,case_increase)

############################ VISUALS ######################################
###########################################################################
###########################################################################

ggplot(data=covid_gdp2, aes(x= reorder(Location,GDP.per.capita), y=GDP.per.capita, fill=gdp_cat)) +
...
xtitle = paste("GDP per capita (U.S. $)\n1 = Poor, 2 = Moderate, 3 = Wealthy\n","<=",cut_20,", <=",cut_40,", <=",cut_59, sep = "")
ggplot(covid_gdp2, aes(y = log(case_increase), x = gdp_cat)) +
  geom_boxplot()+
  ylab("log(New Cases)") +
  xlab(xtitle)+
  ggtitle("New Cases in Quarter 3 across different levels of Wealth (Measured in GDP per Capita)")

poordf = covid_gdp2 %>% filter(gdp_cat == 1)
poor = poordf %>% select(Location, case_increase)

moderatedf = covid_gdp2 %>% filter(gdp_cat == 2)
moderate = moderatedf%>% select(Location, case_increase)

wealthydf = covid_gdp2 %>% filter(gdp_cat == 3)
wealthy = wealthydf %>% select(Location, case_increase)
...
summary(poordf$PopTotal)
summary(moderatedf$PopTotal)
summary(wealthydf$PopTotal)



ggplot(covid_gdp2, aes(y = Change_3_months, x = gdp_cat)) +
  geom_boxplot()+
  ylab("Percent GDP change during quarter 3") +
  xlab(xtitle)+
  ggtitle("New Cases in Quarter 3 across different levels of Wealth (Measured in GDP per Capita)")

poor = poordf %>% select(Change_3_months)
moderate = moderatedf %>% select(Change_3_months)
wealthy = wealthydf %>% select(Change_3_months)
